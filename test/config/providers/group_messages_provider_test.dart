...
void main() {
  group('GroupMessagesProvider Tests', () {
    ...
    group('toMessageModel', () {
      test('converts ChatMessage to MessageModel correctly', () async {
        container = createContainer(
          activePubkey: 'npub1zygjyg3nxdzyg424ven8waug3zvejqqq424thw7venwammhwlllsj2q4yf',
          contacts: testContacts,
        );
        ...
        expect(messageModel.content, 'Test content');
        expect(messageModel.sender.publicKey, 'npub1testkey12345678901234567890');
        expect(messageModel.sender.displayName, 'Alice');
        expect(messageModel.isMe, false);
      });
      ...
      group('when message is a reply', () {
        ...
        setUp(() async {
          ...
          final originalMessageModel = await notifier.toMessageModel(
        ...
        test('has expected reply content', () async {
          expect(messageModel.id, 'reply_message');
          expect(messageModel.content, 'Reply content');
          expect(messageModel.sender.displayName, 'Bob');
          expect(messageModel.isMe, false);
        });

        test('has expected reply to message', () async {
          expect(messageModel.replyTo?.id, 'original_message');
          expect(messageModel.replyTo?.content, 'Original content');
          expect(messageModel.replyTo?.sender.displayName, 'Alice');
          expect(messageModel.replyTo?.isMe, false);
        });
      });
    });
  });
