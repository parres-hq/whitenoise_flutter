      ...
      if (groupData != null) {
        ...
        if (mounted) {
          ...
          ref.showSuccessToast(
            'Chat with ${widget.contact.displayNameOrName} started successfully',
          );
        }
      } else {
  ...
  Future<void> _toggleContact() async {
    ...
    try {
      ...
      if (isCurrentlyContact) {
        ...
        if (mounted) {
          ref.showSuccessToast('${widget.contact.displayNameOrName} removed from contacts');
        }
      } else {
        // Add contact
        await contactsNotifier.addContactByHex(widget.contact.publicKey);
        if (mounted) {
          ref.showSuccessToast('${widget.contact.displayNameOrName} added to contacts');
        }
      }
    } catch (e) {
      if (mounted) {
