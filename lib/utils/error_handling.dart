...
class ErrorHandlingUtils {
  ...
  static Future<String> convertErrorToUserFriendlyMessage({
    ...
    if (error is WhitenoiseError) {
      try {
        _logger.info('${logPrefix}Converting WhitenoiseError to string...');
        final rawErrorMessage = await whitenoiseErrorToString(error: error);
        _logger.info('${logPrefix}WhitenoiseError converted to: $rawErrorMessage');
      ...
      _logger.info('${logPrefix}Unknown error type: ${error.runtimeType}');
      _logger.info('${logPrefix}Error details: $error');
  ...
  static String _handleWrappedException({
    ...
    _logger.info('${logPrefix}Exception string: $exceptionString');
    ...
    if (exceptionString.contains('WhitenoiseErrorImpl')) {
      ...
      if (_containsKeyPackageError(exceptionString, stackTraceString)) {
        ...
        _logger.info('${logPrefix}WhitenoiseErrorImpl details: $exceptionString');
        if (stackTrace != null) {
          _logger.info('${logPrefix}Stack trace: $stackTrace');
        }
      ...
      _logger.info('${logPrefix}Non-WhitenoiseError exception type: ${error.runtimeType}');
      _logger.info('${logPrefix}Error details: $error');
